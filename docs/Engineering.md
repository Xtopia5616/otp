...
> **`code` 类型标注（TS2416）**：`CryptoError` 与 `ApiError`（有子类以各自字面量覆写 `code`）的 `code` 属性须显式标注 `readonly code: string = '...'`。若写 `readonly code = '...'`，TS 推断为字面量类型，子类覆写触发 TS2416。叶子类（`OccConflictError`/`NetworkError`/`SessionRevokedError`/`RateLimitError`/…/`DecryptionError` 等，无子类）可保留字面量推断。运行时值不变。
...
export async function rotateMasterPassword(
  ...
  // BA API 用独立 DB 连接、无法加入 Drizzle 事务，故事务内用 hashPassword(newLak)
  // 直接写 account.password（email/password 路径 providerId='credential'），而非 updatePasswordHash。
  const passwordHash = await hashPassword(params.newLak);

  await db.transaction(async (tx) => {
    // 1. 更新 Better Auth 密码哈希（hashPassword + tx 直写 account.password）
    await tx
      .update(account)
    ...
    // 3. 更新 wrappedDekByMaster（同一 DEK 的新包装）；Blob / wrappedDekByRecovery / passkey_wrap 不动（DEK 恒定）
    await tx
      .update(vault)
  ...
  // 4. 事务提交后吊销其他设备会话（非事务内：经 Drizzle 直接删除 session 表行——
  //    BA 端点 requireHeaders 需会话上下文，不适用于事务后；见 Design §5.2 / server/auth.ts）
