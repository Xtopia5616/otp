...
- [ ] 0.1 `pnpm create svelte@latest` 初始化 SvelteKit skeleton（TS strict + ESLint + Prettier）
- [ ] 0.2 安装运行时依赖（DevSetup §2.2 / §3.1）
- [ ] 0.3 安装开发依赖 + `playwright install chromium`（DevSetup §2.3 / §3.2）
- [ ] 0.4 `shadcn-svelte init`（New York / Zinc / CSS variables）
- [ ] 0.5 配置 `tsconfig.json`（Engineering §1.2：strict + noUncheckedIndexedAccess + verbatimModuleSyntax）
- [ ] 0.6 配置 `.prettierrc` + `eslint.config.js` flat config（Engineering §2.1 / §2.2）
- [ ] 0.7 配置 `svelte.config.js`：adapter-node + CSP（DevSetup §4.2）
- [ ] 0.8 创建 `.env` / `.env.example`（DevSetup §5.1）
- [ ] 0.9 配置 `drizzle.config.ts` + `createdb webotp_dev` + 验证连接
- [ ] 0.10 配置 `vitest.config.ts`（Testing §1.2：unit+integration 双 workspace + coverage 阈值）
- [ ] 0.11 配置 `playwright.config.ts`（Testing §1.3）
- [ ] 0.12 创建目录骨架（Design 附录 A / DevSetup §8）
- [ ] 0.13 配置 `package.json` scripts（DevSetup §9）
- [ ] 0.14 CI 工作流 `.github/workflows/ci.yml`（含 `madge --circular`）
- [ ] 0.15 安装 `madge` devDependency
- [ ] 0.16 空应用冒烟：`pnpm dev` 启动 + `check/lint` 零错误

**验收**: `pnpm dev` 返回 200/302；`check/lint/format:check` 零错误；`drizzle-kit push` 连通 PG；`madge --circular` 无环。详见 [Stage00](./stages/Stage00-项目脚手架与开发环境.md)
...
- [ ] 1.1 `account.ts`：`Account`/`AccountDraft`/`OtpauthParsed`（Architecture §5.1）
- [ ] 1.2 `vault.ts`：Vault API 类型（§9.1，412 响应体裁剪）
- [ ] 1.3 `api.ts`：`AuthParamsResponse`/`RotateKeyRequest`/`PasskeyWrap*`/`Recover*`/`KdfParams`
- [ ] 1.4 `ui.ts`：UIInventory 附录 A 全部 Props
- [ ] 1.5 `errors.ts`：`WebOtpError`/`CryptoError`/`OccConflictError`/`NetworkError`/`SessionRevokedError`/`ApiError`+子类（Engineering §6.1 签名逐字）
- [ ] 1.6 验证 `models/` 零运行时依赖（全 type-only import）
- [ ] 1.7 错误类单测 `tests/unit/models/errors.test.ts`

**验收**: `pnpm check` 通过；`models/` 不 import 其他 `src/lib`；错误类签名与 Engineering §6.1 一致。详见 [Stage01](./stages/Stage01-领域模型与错误基类.md)
...
- [ ] 2.1 `errors.ts`：`DecryptionError`/`KdfError`/`EncodingError`/`FormatError`（extends CryptoError）
- [ ] 2.2 `encoding.ts`：base32/base64 + `serializeBlob`/`parseBlob`（v=1;iv=;ct= 封装）
- [ ] 2.3 `argon2.ts`：`deriveKEK`/`deriveLAK`/`deriveRecoveryVerifier`（hash-wasm）
- [ ] 2.4 `aes-gcm.ts`：`generateIV`/`encryptAesGcm`/`decryptAesGcm` + `encryptAesGcmRandomIv`
- [ ] 2.5 `envelope.ts`：`importKEK`/`generateDEK`/`wrapDek`/`unwrapDek`/`encryptBlob`/`decryptBlob`
- [ ] 2.6 `hkdf.ts`：`deriveKEKPrf`（info=`WebOTP/KEK-PRF/v1`）
- [ ] 2.7 `recovery-key.ts`：`generateRecoveryKey`/`parseRecoveryKey`（96 位/20 字符 4-4-4-4-4）
- [ ] 2.8 `secure-wipe.ts`：`secureWipe`
- [ ] 2.9 测试 fixtures（Testing §2.1/§2.2/§2.3）
- [ ] 2.10 单测 `tests/unit/crypto/`：argon2id/aes-gcm/hkdf/base32/lak/blob-format
- [ ] 2.11 补充 `recovery-key.test.ts`
- [ ] 2.12 补充 `secure-wipe.test.ts`

**验收**: `crypto/**` lines ≥95% / branches ≥90%；AES-GCM 篡改/IV 不复用断言；base32 容错；Blob 拒绝用例；HKDF info 常量；RK 往返 12 字节。详见 [Stage02](./stages/Stage02-密码学原语.md)
...
| 阶段 | 状态 | 任务数 | 完成 |
| :--- | :--- | :--- | :--- |
| Stage 0 — 项目脚手架与开发环境 | ⬜ 未开始 | 16 | 0 |
| Stage 1 — 领域模型与错误基类 | ⬜ 未开始 | 7 | 0 |
| Stage 2 — 密码学原语 | ⬜ 未开始 | 12 | 0 |
